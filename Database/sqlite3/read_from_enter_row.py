...
def read_from_enter_rows():

	car_name = str(input("Enter the car name: "))
	car_version = input("Enter car version: ")

	sql = "SELECT * FROM Car WHERE Name = ? AND Version = ?"

	for row in cor.execute(sql, [(car_name), (car_version)]):
		print(row)

read_from_enter_rows()
